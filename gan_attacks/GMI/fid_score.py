...
def get_activations(dataset, files, model, batch_size=50, dims=2048,
    ...
    for i in tqdm(range(0, len(files), batch_size)):
        ...
        # pred = model(batch)[0]
        pred = model(utils.low2high(batch))
...
def calculate_fid_given_paths(dataset, paths, batch_size, cuda, dims):
    ...
    if dataset == 'celeba':
        #* Modify evaluator model
        model = InceptionResnetV1()
        # model = FaceNet(1000)
        model = torch.nn.DataParallel(model).cuda()
        path_E = './eval_model/FaceNet_pytorch.tar'

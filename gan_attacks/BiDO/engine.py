...
def train_reg(model, criterion, optimizer, trainloader):
    ...
    for batch_idx, (inputs, iden) in pbar:
        inputs, iden = inputs.to(device), iden.to(device)
        #* Change input size from 64 to 112
        inputs = utils.low2high(inputs)
        iden = iden.view(-1)

        #* Only output (bs, 1000) sized out_digit
...
def train_reg_diffR(model, criterion, optimizer, trainloader):
    model.train()
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for batch_idx, (inputs, iden) in pbar:
        ...
        #* Only output (bs, 1000) sized out_digit
        #* Give indens as input as well
        out_digit = model(inputs, iden)
        # feats, out_digit = model(inputs)
        cross_loss = criterion(out_digit, iden)
        # triplet_loss = triplet(feats, iden)
        loss = cross_loss
...
def test_reg_diffR(model, criterion, testloader):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()

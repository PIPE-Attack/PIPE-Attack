...
def get_activations_npz(dataset, files, model, batch_size=50, dims=2048,
                    cuda=False, verbose=False):
    """Calculates the activations of the pool_3 layer for all images.
...
# files = ['../../../datasets/celeba_arcface_128/train/images/000001.jpg']
...
def _compute_statistics_of_path(dataset, path, model, batch_size, dims, cuda):
    if path.endswith('.npz'):
        f = np.load(path)['arr_0']
        # m, s = f['mu'][:], f['sigma'][:]
        act = get_activations_npz(dataset, f, model, batch_size, dims, cuda)
        mu = np.mean(act, axis=0)
        sigma = np.cov(act, rowvar=False)
        # f.close()
        return mu, sigma

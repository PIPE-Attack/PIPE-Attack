...
import os,sys
sys.path.append(os.path.dirname(os.path.dirname(os.getcwd()))+'/TrainNetwork')
from src.Network import Generator
...
class InversionTransformer(TransformerMixin, BaseEstimator):
    ...
    def __init__(self, checkpoint, generator=None, length_of_embedding=512):

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.generator = Generator(length_of_embedding=length_of_embedding) if generator is None else generator
